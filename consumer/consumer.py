...
consumer_ventas = create_kafka_consumer('ventas')
...
while True:
        # Procesar mensajes del topic 'ventas'
    for message in consumer_ventas:
        try:
            data = message.value
            print(f"Mensaje recibido en 'ventas': {data}")

            # Procesa los datos de venta
            cursor.execute("INSERT INTO ventas (total, fecha) VALUES (%s, NOW())", (data['total'],))
            venta_id = cursor.lastrowid

            # Actualiza el inventario y registra los productos vendidos
            for producto in data['productos']:
                cursor.execute("UPDATE inventario SET cantidad = cantidad - %s WHERE nombre = %s", 
                               (producto['cantidad'], producto['nombre']))
                cursor.execute("INSERT INTO productos_vendidos (venta_id, producto, cantidad, precio) VALUES (%s, %s, %s, %s)", 
                               (venta_id, producto['nombre'], producto['cantidad'], producto['precio']))
            db.commit()
            print("Venta procesada e insertada en la base de datos")
        except Exception as e:
            print(f"Error al procesar el mensaje en 'ventas': {e}")
            db.rollback()
